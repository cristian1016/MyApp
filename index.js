...
const transporter = nodemailer.createTransport({
    ...
    port: 587,
    auth: {
        user: 'mercampesinocol@gmail.com',
    ...
    pool.query("INSERT INTO usuario VALUES (?, ?, ?, ?)", [nombres, apellidos, hash, correo],
        (error) => {
            ...
            transporter.sendMail({
                from: 'mercampesinocol@gmail.com',
                to: correo,
...
app.get('/hola', (req, res) => {
    ...
    const data = ['Artículo 1', 'Artículo 2', 'Artículo 3'];

    // Verifica si existe la sesión llamada 'correo' y si no ha expirado
    if (req.session.correo) {
        // Retorna la plantilla 'hola' al cliente con la información de los artículos
        // y se mostrarán los nombres del usuario en el nav
        return res.render('hola', { nombres: req.session.nombres, hola: data });
    } else {
        // Retorna la plantilla 'hola' al cliente con la información de los artículos
        // y no se mostrarán los nombres del usuario en el nav, ya que no está logueado
        return res.render('hola', { nombres: undefined, hola: data });
    }
});
